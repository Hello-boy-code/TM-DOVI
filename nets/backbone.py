...
#制作不同的层，将其压入Sequential
    ...
    def forward(self, x):
        ...
        return y
# pretrained:是否使用主干网络权重

#创建并返回一个 HRNet（高分辨率网络）分类模型对象，并且可以选择加载预训练的权重。
def hrnet_classification(pretrained=False, backbone='hrnetv2_w18'):
    ...
    if pretrained:
        ...
        #下载权重，model_dir：保存的位置
        state_dict = load_state_dict_from_url(model_urls[backbone], model_dir="./model_data")
        #这时的state_dict是一个字典形式
        #这时可以直接加载入模型中
        #或者将其保存为path形式,torch.save(model.state_dict(), 'model_weights.pth')
        #加载path，model.load_state_dict(torch.load('model_weights.pth'))，将path加载为字典形式
        #将权重加载到model
