...
'''
计算F_score的值，手动计算 ---评价模型的性能
第二种方法：通过from sklearn.metrics import f1_score
_f_score = f1_score(labels.cpu(), predicted.cpu())
'''
def f_score(inputs, target, beta=1, smooth = 5e-5, threhold = 0.5):
    ...
    #--------------------------------------------#
    #   计算dice系数,计算F_score的值
    #--------------------------------------------#
    temp_inputs = torch.gt(temp_inputs, threhold).float()
    tp = torch.sum(temp_target * temp_inputs, axis=[0, 1])  # 按位相乘，保留得到的空间size
