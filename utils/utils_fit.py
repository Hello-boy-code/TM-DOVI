...
#Functional function
#Set the number of epochs for each round
def fit_one_epoch(model_train, model, loss_history, eval_callback, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, Epoch, cuda, dice_loss, focal_loss, cls_weights, num_classes, fp16, scaler, save_period, save_dir, local_rank=0):
    ...
    for iteration, batch in enumerate(gen):
        ...
        if not fp16:
            ...
            with torch.no_grad():
                ...
                #   computer f_score
                #-------------------------------#
                #Calculate the value of F1_score
    ...
    if local_rank == 0:
        global minmiou
        if 'minmiou' not in globals():
            minmiou = 0
        pbar.close()
        print('Finish Validation')
        loss_history.append_loss(epoch + 1, total_loss / epoch_step, val_loss / epoch_step_val)
        miou = eval_callback.on_epoch_end(epoch + 1, model_train,epoch)
